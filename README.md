...
- **Claude Code skill — official Stellar skill:** listed on [skills.stellar.org](https://skills.stellar.org/) (source: [ctg-skills](https://github.com/Dione-b/caatinga-skill)). Teaches Claude Code Caatinga's rules (no raw `stellar` deploys, no manual `caatinga.artifacts.json`/bindings edits, `--source` as an identity alias only). Install with `/plugin marketplace add Dione-b/caatinga-skill` then `/plugin install caatinga-skill@caatinga-skill`.
