...
> [!WARNING]
> **Alpha software.** APIs and config formats may change before `v1.0.0`. `latest` and `next` both track `2.1.0`. Pin an exact version in apps and check the [CHANGELOG](./packages/cli/CHANGELOG.md) before upgrading.
