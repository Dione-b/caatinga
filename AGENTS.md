...
Published on npm:
- **`next`**: **`2.4.1`** (`@caatinga/cli`, `@caatinga/core`, `@caatinga/client`, `@caatinga/zk`)
...
- `2.4.1`: `CARGO_TARGET_DIR` WASM resolution, doctor deploy check via `defaultNetwork`, skip bindings on minimal deploy, zk-minimal deploy script fixes
