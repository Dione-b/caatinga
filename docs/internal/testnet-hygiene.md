...
4. **`ctg read --summary`** — compact output for large array payloads in daily smoke scripts.
5. **`smoke.useFreshSymbol`** — when `true`, Caatinga injects a fresh UUID `symbol` arg on each smoke read.
6. **`ctg regression`** — run the full testnet pipeline locally; see [Testing](./testing.md#deploy-regression-testnet).
