...
- **Status:** alpha. Release atual `2.2.1` (linha `2.x`, bump major a partir de `0.2.4`). Destaques: `caatinga status`, deploy com geração automática de bindings, `@caatinga/client/react`, gate `ContractNotDeployed` no template counter, overrides de install no template counter.
